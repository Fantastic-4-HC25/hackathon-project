// components/dashboard/ListingsSection.jsx
"use client";

export default function ListingsSection({ listings, loading }) {
    ...
    return (
        <div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
                {listings.map((p) => (
                    <article key={p.id} className="border rounded-lg p-4 bg-background">
                        <h3 className="font-semibold">{p.title}</h3>
                        <p className="text-sm text-muted-foreground line-clamp-3">{p.description}</p>
                        <div className="mt-2 text-sm">
                            <div><strong>Type:</strong> {p.type}</div>
                            <div><strong>Location:</strong> {p.location}</div>
                            <div><strong>Price:</strong> ₱{Number(p.price || 0).toLocaleString()}</div>
